...
def test_actualizar_producto(client):
    producto = {
        "nombre": "Café",
        "descripcion": "Café Arábica de alta calidad",
        "precio": 2.5,
        "stock": 100
    }

    producto_id = client.post("/productos", json=producto).json()["id"]
...
def test_comprar_producto(client):
    ...
    new_item_id = client.post("/productos", json=producto).json()["id"]
...
def test_reponer_stock(client):
    producto = {
        "nombre": "Café en granos",
        "descripcion": "Café Arábica de altazzz calidad en grano",
        "precio": 3.0,
        "stock": 9
    }
